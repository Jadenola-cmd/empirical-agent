...
export function matrixInverse(matrix, ridgeLambda = 0) {
  ...
  if (ridgeLambda > 0) {
    for (let i = 0; i < n; i++) {
      workingMatrix[i][i] += ridgeLambda;
    }
  }
  
  const augmented = workingMatrix.map((row, i) => {
    const newRow = [...row];
    for (let j = 0; j < n; j++) {
      newRow.push(i === j ? 1 : 0);
    }
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (Math.abs(pivot) < 1e-10) {
      // 如果主元太小，尝试岭回归
      if (ridgeLambda === 0) {
        return matrixInverse(matrix, 1e-8);
      }
      throw new Error('矩阵奇异，无法求逆（请检查变量是否存在多重共线性）');
    }
