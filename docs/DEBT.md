...
**~~DID 稳健性检验的安慰剂检验固定 `n_placebo=100`，未根据数据规模动态调整~~（已解决，2026-06-15）**
2026-06-15 实机QA在 Railway 测试环境复现：500行/50个体的测试数据，单独勾选 `did_robustness`（含 `control_vars`）即把 CPU 冲到 2vCPU 以上并触发 90 秒超时熔断（本地约14秒，Railway资源更弱导致放大）。根因是每次安慰剂迭代都重建完整 PanelOLS。已改为预计算双向固定效应去均值（`_two_way_demean`）+ 每次迭代仅做一次 `lstsq`，`n_placebo=100` 总耗时从约14秒降到约0.5秒，系数误差<1e-6。详见 `docs/CHANGELOG.md` 2026-06-15（续）性能修复条目。
